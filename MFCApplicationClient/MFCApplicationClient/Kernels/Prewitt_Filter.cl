__kernel void PrewittKernel(__read_only image2d_t input_image,
                           __write_only image2d_t output_image,
                           int width,
                           int height,
                           int count_channels,
                           sampler_t sampler,
                           __constant int* prewittGx,
                           __constant int* prewittGy)
{
	...
	for (int c = 0; c < 3; c++)
    {
		...
		for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                Gx += prewittGx[(i + 1) * 3 + (j + 1)] * pixel[c];
                Gy += prewittGy[(i + 1) * 3 + (j + 1)] * pixel[c];
			}
		}
