__kernel void RobertsKernel(__read_only image2d_t input_image,
                          __write_only image2d_t output_image,
                          int width,
                          int height,
                          int count_channels,
                          sampler_t sampler,
                          __constant int* robertsGx,
                          __constant int* robertsGy)
{
	...
	for (int c = 0; c < 3; c++)
    {
		...
		for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                Gx += robertsGx[i * 2 + j] * pixel[c];
                Gy += robertsGy[i * 2 + j] * pixel[c];
			}
		}
